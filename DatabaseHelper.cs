...
public static class DatabaseHelper
{
    ...
    /// <summary>
    /// Выполняет обычный запрос, который возвращает одно значение: COUNT, SUM, цена и т.п.
    /// </summary>
    ...
    /// Например: qryAvailableScooters или qryUserRentals.
    /// </summary>
    ...
    /// <summary>
    /// Выполняет сохранённый запрос Access по имени и возвращает одно значение.
    /// Например: qryScooterPricePerHour возвращает цену самоката за час.
    /// </summary>
    public static object? ExecuteScalarByName(OleDbConnection connection, string queryName, params OleDbParameter[] parameters)
    {
        bool wasOpen = connection.State == ConnectionState.Open;
        if (!wasOpen)
            connection.Open();
